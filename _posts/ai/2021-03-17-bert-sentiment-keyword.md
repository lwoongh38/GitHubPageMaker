...
이유를 세 가지 정도로 생각해봤는데,  
첫번째는 키워드 추출과정이다. 키워드를 추출하는 업무는 아무래도 문장이 어떻게 토큰화 되느냐가 중요하다. 이번 프로젝트에서는 시간관계상 코버트에서 제공하는 토크나이저를 사용했는데 실험의 정확도를 위해서는 다른 토크나이저를 사용해서 비교해봐야 결과값을 제대로 판단할 수 있을 것 같습니다.  
